...
namespace LightControl
{
	...
	public class CCSLightControlManager : IDisposable
	{
		...
		{
			_heartbeatInterval = (probeInterval.HasValue) ? probeInterval.Value : 200;
		...
		{
			if (_heartBeatWorker != null && _heartBeatWorker.IsBusy)
			...
			if (model != null)
			{
				...
				foreach (var channelCommand in model.ChannelCommands)
				{
					...
					else
					{
						...
						var replyMsg = sendData(data) + ";" + channelCommand.Value.Instruction;
		...
		private string getCheckSum(string command)
		{
			...
			var checksumChars = sum.ToString("X").ToCharArray();
			//反轉
			char[] reverseChars = new char[checksumChars.Length];
			int index = 0;
			for (int i = checksumChars.Length - 1; i >= 0; i--)
			{
				reverseChars[index] = checksumChars[i];
				index++;
			}
			//取 2 碼
			var take2Chars = new char[2];
			take2Chars[0] = reverseChars[1];
			take2Chars[1] = reverseChars[0];
			return new string(take2Chars);
