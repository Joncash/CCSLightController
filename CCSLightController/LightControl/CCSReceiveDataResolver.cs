...
namespace LightControl
{
	public class CCSReceiveDataResolver
	{
		...
		{
			/* Message 組成
			 * header (1bit) + channel (2bit) + receiveCommand + checksum + Delimiter + Instruction
			...
			onOff = (onOff == "1") ? "true" :
					(onOff == "0") ? "false" :
					onOff;
			var instructionParts = message.Split(';');
			var instruction = (instructionParts.Length > 1) ? instructionParts[1] : "";
